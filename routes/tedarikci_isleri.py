...
from models import db, Tedarikci, SatinAlma, TedarikciOdeme
...
def tedarikci_detay(id):
    ...
    net_tl = sum(alim.tutar * GUNCEL_KURLAR.get(alim.para_birimi, 1.0) for alim in t.satin_almalar) - sum(o.tutar * o.kur_degeri for o in t.odenenler)
    return render_template('tedarikci_detay.html', tedarikci=t, toplam_tl=round(net_tl, 2), toplam_usd=round(net_tl / GUNCEL_KURLAR.get('USD', 1.0), 2), kurlar=GUNCEL_KURLAR)
...
def tedarikci_odeme_yap():
    ...
    db.session.add(TedarikciOdeme(tutar=float(request.form.get('tutar') or 0), birim=birim, aciklama=request.form.get('aciklama'), kur_degeri=GUNCEL_KURLAR.get(birim, 1.0) if birim != 'TL' else 1.0, tedarikci_id=t_id))
    db.session.commit()
    return redirect(url_for('tedarikci.tedarikci_detay', id=t_id))
