...
from models import db, Musteri, IsKaydi, Odeme
...
def satislar():
    ...
    for m in aktif_musteriler:
        for i in m.isler:
            if i.durum == 'Devam Ediyor': devam_eden_is_sayisi += 1
    ...
    return render_template('satislar.html', alacak=round(toplam_alacak_tl, 2), is_sayisi=devam_eden_is_sayisi, yaklasan_isler=yaklasan_isler, kurlar=GUNCEL_KURLAR, t_musteri=Musteri.query.count(), a_musteri=len(aktif_musteriler), biten_is=IsKaydi.query.filter_by(durum='Teslim Edildi').count(), ay_ciro=bu_ayki_ciro)
...
def musteri_ekle():
    db.session.add(Musteri(ad_soyad=request.form.get('ad_soyad'), telefon=request.form.get('telefon'), isyeri_adi=request.form.get('isyeri_adi'), isyeri_adresi=request.form.get('isyeri_adresi'), durum='Aktif'))
    db.session.commit()
...
def musteri_detay(id):
    ...
    return render_template('musteri_detay.html', musteri=m, toplam_tl=round(net_tl, 2), toplam_usd=round(net_tl / GUNCEL_KURLAR.get('USD', 1.0), 2), kurlar=GUNCEL_KURLAR)
...
def musteri_duzenle(id):
    ...
    if request.method == 'POST':
        musteri.ad_soyad, musteri.telefon, musteri.isyeri_adi, musteri.isyeri_adresi = request.form.get('ad_soyad'), request.form.get('telefon'), request.form.get('isyeri_adi'), request.form.get('isyeri_adresi')
        db.session.commit()
...
def musteri_sil(id):
    if 'logged_in' not in session: return redirect(url_for('genel.index'))
    musteri = Musteri.query.get_or_404(id)
    if musteri.isler or musteri.odemeler: musteri.durum = 'Pasif'
    else: db.session.delete(musteri)
    db.session.commit()
    return redirect(url_for('musteri.musteriler'))
...
def is_kaydet():
    m_id, is_adi, t_bedel, p_birimi = int(request.form.get('musteri_id')), request.form.get('is_tanimi'), float(request.form.get('toplam_bedel') or 0), request.form.get('para_birimi')
    db.session.add(IsKaydi(is_tanimi=is_adi, toplam_bedel=round(t_bedel, 2), para_birimi=p_birimi, teslim_tarihi=request.form.get('teslim_tarihi'), durum='Devam Ediyor', musteri_id=m_id))
    a_kapora = float(request.form.get('alinan_kapora') or 0)
    if a_kapora > 0:
        k_birimi = request.form.get('kapora_birimi')
        db.session.add(Odeme(tutar=round(a_kapora, 2), birim=k_birimi, aciklama=f"Kapora ({is_adi})", kur_degeri=GUNCEL_KURLAR.get(k_birimi, 1.0) if k_birimi != 'TL' else 1.0, musteri_id=m_id))
    db.session.commit()
...
def musteri_odeme_ekle(m_id):
    ...
    db.session.add(Odeme(tutar=round(float(request.form.get('tutar') or 0), 2), birim=birim, aciklama=request.form.get('aciklama'), kur_degeri=kur, musteri_id=m_id))
    db.session.commit()
    return redirect(url_for('musteri.musteri_detay', id=m_id))
